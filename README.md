...
这是一个基于 Qt 开发的**赛博朋克风格**进程调度算法可视化模拟器，用于演示和理解操作系统中的进程调度机制。项目使用 C++17 和 Qt 框架实现，提供了图形化界面来展示进程的生命周期、CPU 执行过程以及进程在 CPU 与就绪队列之间的状态流转。
...
- **甘特图数据**: 记录执行历史用于可视化
- **统计信息**: 计算平均等待时间、周转时间等
- **单步执行**: 以 tick 为单位推进调度过程
- **回队可视化**: 进程时间片用完后会先回到就绪队列，再参与下一轮调度
...
- **能量流动**: 水平扫描光带
- **多层边框**: 4 层渐变发光
- **实时状态**: 显示当前或刚刚执行过的进程信息
- **状态标签**: 区分“运行中”、“已回就绪队列”、“刚执行完成”等状态
...
2. **调度控制**
   - 单步执行：一次推进 1 个调度 tick
   - 自动执行：连续执行调度过程
   - 重置：恢复初始状态
   - 算法对比：对比 5 种调度算法性能

3. **可视化展示**
   - CPU 执行单元显示当前或刚执行过的进程
   - 就绪队列实时显示等待中的进程，以及时间片结束后重新入队的过程
   - 甘特图动态更新
   - 统计面板实时更新
   - 调度步骤文字解释
...
4. **开始调度**:
   ...
   - 或点击"▶ 自动"按钮自动运行
5. **查看状态**: 
   - CPU 执行单元显示当前或刚执行过的进程，并标注当前状态
   - 就绪队列显示等待进程，以及时间片结束后重新插入的进程
   - 甘特图展示执行历史
   - 统计面板显示性能指标
