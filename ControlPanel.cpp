...
void ControlPanel::setupUI() {
    ...
    layout->setSpacing(9);
    buildAlgorithmSection(layout);
    layout->addWidget(createSeparator());
    buildInputSection(layout);
    layout->addWidget(createSeparator());
    buildSpeedSection(layout);
    layout->addWidget(createSeparator());
    buildControlSection(layout);
    layout->addStretch();
    connectSignals();
    onAlgorithmChanged(m_algoCombo->currentIndex());
}

void ControlPanel::buildAlgorithmSection(QVBoxLayout* layout) {
...
void ControlPanel::buildInputSection(QVBoxLayout* layout) {
    const QString dimStyle = QString(
    ...
    m_priorityLabel->setStyleSheet(dimStyle);
    ...
    timeLabel->setStyleSheet(dimStyle);
...
void ControlPanel::buildSpeedSection(QVBoxLayout* layout) {
...
void ControlPanel::buildControlSection(QVBoxLayout* layout) {
...
QLabel* ControlPanel::createSeparator() {
...
Algorithm ControlPanel::algorithmFromIndex(int index) const {
    switch(index) {
...
void ControlPanel::onAlgorithmChanged(int index) {
    const Algorithm algo = algorithmFromIndex(index);
...
Algorithm ControlPanel::selectedAlgorithm() const { return algorithmFromIndex(m_algoCombo->currentIndex()); }
